...
static void my_application_activate(GApplication* application) {
  ...
  if (use_header_bar) {
    ...
    gtk_header_bar_set_title(header_bar, "MAICENTA");
    gtk_header_bar_set_show_close_button(header_bar, TRUE);
    gtk_window_set_titlebar(window, GTK_WIDGET(header_bar));
  } else {
    gtk_window_set_title(window, "MAICENTA");
  }

  // Resolve the bundled Flutter asset relative to the executable so the icon
  ...
  if (executable_path != nullptr) {
    ...
    g_autofree gchar* icon_path =
        g_build_filename(executable_directory, "data", "flutter_assets",
                         "assets", "branding", "maicenta-symbol.png", nullptr);
